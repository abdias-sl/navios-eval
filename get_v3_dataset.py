...
for q_type, num_documents in question_types.items():
    # Filter for this question type
    filtered_df = df[df['question_types'].apply(lambda x: q_type in x)]
    
    # Get docIds that have at least 5 questions of this type
    valid_docids = filtered_df.groupby('docId').size()
    valid_docids = valid_docids[valid_docids >= 3]
    
    if len(valid_docids) > 0:
        # Take the first 'num_documents' documents
        selected_docids = valid_docids.head(num_documents).index
        
        print(f"\n{q_type.title()} Question Examples:")
        
        for docid in selected_docids:
            examples = filtered_df[filtered_df['docId'] == docid].head(3)
            ...
            for _, row in examples.iterrows():
                ...
                if row['docId'] not in saved_images:
                    try:
                        ...
                        # Save image
                        image_path = f"./datasets/docVQA/bigger-subset/{row['docId']}.png"
                        img.save(image_path)
        ...
        print(f"\nNo document found with 5+ {q_type} questions")
...
with open("./datasets/docVQA/bigger-subset/selected_examples.json", "w") as f:
    json.dump(output_data, f, indent=2)

print(f"\n✅ Saved {len(selected_examples)} examples to ./datasets/docVQA/bigger-subset/selected_examples.json")
print(f"✅ Saved {len(saved_images)} unique images to ./datasets/docVQA/bigger-subset")
